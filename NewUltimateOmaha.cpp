...
double calcTotalInitialBets(std::vector<SideBet*>&, double);
...
void validateSideBets(std::vector<SideBet*>&, std::vector<double>&, double, double&);
...
double playUltimateHoldem(double moneyIn)
{
	...
	do
	{
		...
		if (initialBets.empty()) {
			ante = validatePreflopBet(ultTexBlindBet, moneyIn);
			initialBets.push_back(ante);
		}
		else {
			ante = initialBets[0];
		}
		...
		validateSideBets(sideBets, initialBets, moneyIn, totalBet);
		...
		double totalInitialBets = calcTotalInitialBets(sideBets, ante);
		playAgain = tolower(validatePlayAgain(moneyIn, totalInitialBets));
		if (playAgain != 'b') {
			initialBets.clear();
		}
...
void validateSideBets(std::vector<SideBet*>& sideBets, std::vector<double>& initialBets, double moneyIn, double& totalBet)
{

	
	auto initialBetsIt = initialBets.begin();
	bool useInitialBets = false;
	if (initialBets.size() > 1) {
		initialBetsIt++;
		useInitialBets = true;
	}
	...
	for (auto it = sideBets.begin(); it != sideBets.end(); it++)
	{
		...
		{
			bool validBetChoice = false;
			do
			{
				if (!useInitialBets) {


					cout << "Do you want to make a " << (*it)->getName() << "? Enter y for yes or n for no." << endl;
				...
				else if (sideBetChoice == 'y')
				{
					...
					if (!useInitialBets) 
					{
						initialBets.push_back(sideBetAmount);
					}
					
				}
				else if (sideBetChoice == 'n')
				{
					validBetChoice = true;
					(*it)->setBetChoice('n');
					(*it)->setBetAmount(0);
					if (!useInitialBets)
					{
						initialBets.push_back(0);
					}
				}
				else {
					cout << "That is not a valid choice. Please try again." << endl;
				}
			} while (!validBetChoice);
		}
		else
		{
			(*it)->setBetChoice('n');
		}
		if (useInitialBets && initialBetsIt != initialBets.end()) {
			initialBetsIt++;
		}
...
double calcTotalInitialBets(std::vector<SideBet*>& sideBets, double ante)
{
	double result = ante * ANTE_MULTIPLIER;
	for (auto it = sideBets.begin(); it != sideBets.end(); it++)
	{
		result += (*it)->getBetAmount();
	}
...
char validatePlayAgain(double moneyIn, double totalInitialBets)
{
	...
	do {
		...
		if (moneyIn >= 0.01 * ULT_NUM_BETS)
		{
			...
			else if (playAgain == QUIT_CHOICE) {
				validChoice = true;
			}
			else if (playAgain == SAME_BETS_CHOICE && moneyIn >= totalInitialBets) {
			
				validChoice = true;
			}
			else if (playAgain == SAME_BETS_CHOICE) {
				cout << "You do not have enough money to play another hand with the same initial bets." << endl;
			}
		}
		else
		{
